...
const OrganizationClient = class extends EventEmitter {
  ...
  async query(chaincodeId, chaincodeVersion, fcn, ...args) {
    ...
    let result = await this._channel.queryByChaincode(request).payload;
    return unmarshalResult(result);
  }

  async queryTransaction(tx_id) {
    let result = await this._channel.queryTransaction(tx_id);
    console.log(JSON.stringify(result.transactionEnvelope.payload.data));
  }
...
async function getSubmitter(
  ...
  try {
    ...
    let ca;
    if (this != null && this._caConfig != null) {
      ca = new CAClient(url, {
        verify: false
      }, this._caConfig.name);
    } else {
